...
export const userPlugin = new Elysia({
	...
	.get(
		...
		async ({ user, params: { role } }) => {
			return UserService.fetchAssignedUsers(user.id, role === "any" ? undefined : role);
		},
		{
			user: true,
			params: t.Object({
				role: t.Optional(
					t.UnionEnum(["operator", "admin", "client", "supervisor", "any"] as const),
				),
			}),
			detail: {
				summary: "Get Assigned Users",
				description: "Get users assigned to the current user (or admin's users if not admin)",
			},
		},
	)
	.delete(
