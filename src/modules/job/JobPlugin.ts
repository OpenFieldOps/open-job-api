...
export const jobPlugin = new Elysia({
	...
	.put(
		"/apply/:id",
		({ params: { id }, user }) => JobService.applyJob(user, id),
		{
			params: paramsWithId(),
			role: "operator",
			detail: {
				summary: "Apply for Job",
				description: "Apply for a Job by its ID.",
			},
		},
	)
	...
	.get(
		"/:id/task/",
		async ({ params: { id }, user }) => JobTaskService.getJobTasks(id, user.id),
		{
			user: true,
			params: t.Object({
				id: t.Number(),
	...
	.patch(
		"/task",
		async ({ body, user }) => JobTaskService.updateJobTask(body, user.id),
		{
			user: true,
	...
	.delete(
		"/task/:taskId",
		async ({ params: { taskId }, user }) =>
			JobTaskService.deleteJobTask(taskId, user.id),
		{
			user: true,
			params: t.Object({
				taskId: t.Number(),
